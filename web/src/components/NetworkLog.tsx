...
export function NetworkLog({ instanceId, refreshTick }: Props) {
  ...
  if (!h) return <div className="text-sm text-neutral-500">{t("Loading…")}</div>;
  ...
  return (
    <div className="flex flex-col gap-4 h-full overflow-auto">
        ...
        <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-neutral-400">
              {h.current_allowlist.length === 0 ? (
                <span className="text-amber-400">
                  {t("allowlist empty — nothing actually reachable")}
                </span>
              ) : (
                <>
                  {t("allowlist:")} {h.current_allowlist.join(", ")}
                </>
              )}
            </span>
          )}
          {h.current_policy === "open" && (
            <span className="text-[11px] text-red-400">
              {t("sandbox firewall is OFF — every host reachable")}
            </span>
          )}
          {h.current_policy === "off" && (
            <span className="text-[11px] text-neutral-500">
              {t("fully isolated — sandbox denies all network")}
            </span>
          )}
        </div>
      ...
      <section className="flex-1 min-h-0">
        <div className="text-[10px] text-neutral-500 uppercase tracking-wide mb-2">
          {t("Recent loops with tool activity (window {window})", {
            window: h.window,
          })}
        </div>
        {h.entries.length === 0 ? (
          <div className="text-sm text-neutral-600">
            {t("No tool runs recorded in the last {window} expands.", {
              window: h.window,
            })}
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-xs font-mono text-neutral-300">
                    {t("loop")} {String(e.loop_n).padStart(5, "0")}
                  </span>
                  <span
                  ...
                  <span className="text-[10px] text-neutral-500">
                    {e.tool_runs.length === 1
                      ? t("{count} tool run", { count: e.tool_runs.length })
                      : t("{count} tool runs", { count: e.tool_runs.length })}
                  </span>
                </div>
